defmodule SnowflexTest do
  ...
  alias Snowflex.EctoAdapter.Connection, as: SQL
  ...
  test "order by" do
    ...
    for dir <- [:asc_nulls_first, :asc_nulls_last, :desc_nulls_first, :desc_nulls_last] do
      assert_raise Ecto.QueryError, ~r"#{dir} is not supported in ORDER BY in Snowflake", fn ->
        TestSchema |> order_by([r], [{^dir, r.x}]) |> select([r], r.x) |> plan() |> all()
      end
    end
  end
  ...
  describe "windows" do
    ...
    test "window raises an exception" do
      assert_raise RuntimeError, ~r/Snowflex adapter does not support window functions/, fn ->
        query =
          TestSchema
          |> select([r], r.x)
  ...
  test "join with hints" do
    assert TestSchema
           |> join(:inner, [p], q in TestSchema2, hints: ["USE INDEX FOO", "USE INDEX BAR"])
           |> select([], true)
           |> plan()
           |> all() ==
  ...
  test "join with query interpolation" do
    ...
    query = from(p in TestSchema, left_join: c in ^inner, select: {p.id, c.id}) |> plan()
  ...
  #   assert_raise ArgumentError, ~r/Snowflake adapter does not support constraints/, fn ->
  #     execute_ddl(
  ...
  #   assert_raise ArgumentError, "Snowflake adapter does not support where in indexes", fn ->
  ...
  #   assert_raise ArgumentError, "Snowflake adapter does not support check constraints", fn ->
  ...
  #   end
  #
  #   assert_raise ArgumentError, "Snowflake adapter does not support exclusion constraints", fn ->
