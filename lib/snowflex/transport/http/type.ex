defmodule Snowflex.Transport.Http.Type do
  ...
  # Fallback for when we don't have type information but have column name
  def decode(value, %{column: column_name}) when is_binary(value) and is_binary(column_name) do
    column_name = String.downcase(column_name)

    if String.contains?(column_name, "id") and integer_string?(value) do
      # If column name contains "id" and looks like an integer, convert to integer
  ...
  # Helper to check if a string is integer-like
