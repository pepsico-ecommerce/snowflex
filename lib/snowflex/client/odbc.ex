defmodule Snowflex.Client.ODBC do
  @moduledoc """
  ...
  def sql_query(pid, statement, opts \\ []) do
    if Process.alive?(pid) do
      ...
      |> IO.inspect(label: "sql_query #{statement}")
    else
  ...
  def param_query(pid, statement, params, opts \\ []) do
    if Process.alive?(pid) do
      ...
      |> IO.inspect(label: "param_query #{statement}")
    else
      {:error, %Error{message: :no_connection}}
    end
  end
  ...
  defp connection_string(connection_args) do
    driver = Keyword.get(connection_args, :driver, Application.get_env(:snowflex, :driver))
